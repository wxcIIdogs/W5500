...
/*******************************************************************/
/* Operation mode bits */
#define VDM		0x00		  //可变数据长度模式
#define FDM1	0x01		  //固定1字节数据长度模式
#define	FDM2	0x02		  //固定2字节
#define FDM4	0x03		  //固定4字节
...
extern u8 Sub_Mask[4];	//子网掩码 
extern u8 Phy_Addr[6];	//物理地址(MAC) 
extern u8 IP_Addr[4];	//本机IP地址 

extern u8 S0_Port[2];	//端口0的端口号(5000) 
extern u8 S0_DIP[4];	//端口0目的IP地址 
extern u8 S0_DPort[2];	//端口0目的端口号(6000) 

extern u8 UDP_DIPR[4];	//UDP(广播)模式,目的主机IP地址
extern u8 UDP_DPORT[2];	//UDP(广播)模式,目的主机端口号


// 端口的运行模式 
extern u8 S0_Mode;	//端口0的运行模式,0:TCP服务器模式,1:TCP客户端模式,2:UDP(广播)模式
#define TCP_SERVER		0x00	//TCP服务器模式
#define TCP_CLIENT		0x01	//TCP客户端模式 
#define UDP_MODE		0x02	//UDP(广播)模式 

//端口的运行状态 
extern u8 S0_State;	//端口0状态记录,1:端口完成初始化,2端口完成连接(可以正常传输数据) 
#define S_INIT			0x01	//端口完成初始化 
#define S_CONN			0x02	//端口完成连接,可以正常传输数据 

// 端口收发数据的状态 
extern u8 S0_Data;		//端口0接收和发送数据的状态,1:端口接收到数据,2:端口发送数据完成 
#define S_RECEIVE		0x01		//端口接收到一个数据包 
#define S_TRANSMITOK	0x02		//端口发送一个数据包完成 

// 端口数据缓冲区 
extern u8 Rx_Buffer[2048];	//端口接收数据缓冲区 
extern u8 Tx_Buffer[2048];	//端口发送数据缓冲区 
...
extern u8 Detect_Gateway(void);    //检查网关服务器
extern void Socket_Init(SOCKET s);            //指定Socket(0~7)初始化
extern u8 Socket_Connect(SOCKET s);//设置指定Socket(0~7)为客户端与远程服务器连接
extern u8 Socket_Listen(SOCKET s); //设置指定Socket(0~7)作为服务器等待远程主机的连接
extern u8 Socket_UDP(SOCKET s);    //设置指定Socket(0~7)为UDP模式
extern u16 Read_SOCK_Data_Buffer(SOCKET s, u8 *dat_ptr);          //指定Socket(0~7)接收数据处理
extern void Write_SOCK_Data_Buffer(SOCKET s, u8 *dat_ptr, u16 size); //指定Socket(0~7)发送数据处理
